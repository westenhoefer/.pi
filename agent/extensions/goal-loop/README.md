...
- The [session-owned subagent supervisor](../simple-subagents/README.md) cooperates explicitly. `subagent_start` is allowed only in an approved working round; `subagent_status`, `subagent_steer`, and `subagent_stop` do not disarm the loop. At a dependency barrier, checkpoint `waiting`: the countdown stays suspended. Matching completion messages resume result handling in the **same round** without increasing the round count, refreshing deadlines, or granting the next manual approval. Every owned result must arrive before `continue`/`done`, followed by a fresh final checkpoint. Process completion or queue acceptance alone does not consume a result. `/loop stop` never kills children.
- `job_start` or any legacy `subagent` tool call still disarms automatic continuation conservatively (including legacy management queries). The model can finish independent work normally; children/jobs retain their native notification and cancellation behavior. Restart explicitly after their results are consumed.
- External extension input/custom messages other than exact owned subagent results also disarm the loop, avoiding competing automation. Duplicate or old-loop child messages conservatively stop a currently active loop rather than approving/reviving it. A session-branch watermark catches idle custom messages that Pi does not forward to extension message hooks; any unexpected branch change during countdown conservatively stops it, even if an extension only appended bookkeeping. Arbitrary extension slash commands bypass Pi's input hook; not all are detectable. Busy/pending work prevents a countdown firing and requires a fresh checkpoint. Custom extensions that alter runtime behavior need their own compatibility assessment.
...
The installed pi-subagents package is retained but unloaded. Its goal missions are not used by this loop. The small session-owned supervisor communicates through the contract in `../simple-subagents/contract.ts`; it does not own loop timers or approval.
...
Tests cover delay and round-count boundaries, the automatic deadline and unbounded manual waits, checkpoints, failures, steering, typing, manual approval/resume, exact child-result correlation, same-round delegated waits, stale completion notices, explicit cancellation, lifecycle cleanup, background handoff, and unsafe start rejection. Timers are advanced with Node's mock clock: no minute-long sleeps, filesystem fixtures, or live model usage. Loader integration verifies registration against the installed SDK, not a complete real agent session. After `/reload`, manually check the countdown, steering, and stop behavior with a small read-only goal. Do not use a destructive task for a smoke test.
